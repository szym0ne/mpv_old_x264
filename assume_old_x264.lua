function old_x264()
    local pf = mp.get_property("video-params/pixelformat") or ""
    local hw_pf = mp.get_property("video-params/hw-pixelformat") or ""

    if string.match(pf, "yuv444p10") or string.match(hw_pf, "yuv444p10") then
...
mp.register_event("file-loaded", old_x264)
